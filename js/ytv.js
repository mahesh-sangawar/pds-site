...
(function (win, doc) {
    'use strict';
    var apiKey = 'AIzaSyBxFB2HIjH3T9VBuioaIpOFpd2pnDqf_OE';
                ...
                compileList: function (data) {
                    if (data && data.items.length > 0) {
                        utils.ajax.get(utils.endpoints.userInfo(), function (userInfo) {
                            ...
                            var subAnim = new CountUp("totalSubscribers", 0, user.subscribers);
                            if (!subAnim.error) {
                                subAnim.start();
                            } else {
                                console.error(subAnim.error);
                            }
                            var viewAnim = new CountUp("totalViews", 0, user.views);
                            if (!viewAnim.error) {
                                viewAnim.start();
                            } else {
                                console.error(viewAnim.error);
                            }
                            settings.channelId = userInfo.items[0].id;
                            if (settings.currentPlaylist) user.title += ' &middot; ' + (settings.currentPlaylist);
                            if (settings.sortList) videos.sort(function (a, b) {
                            ...
                            if (settings.shuffleList) {
                                videos = function () {
                                    for (var j, x, i = videos.length; i; j = Math.floor(Math.random() * i), x = videos[--i], videos[i] = videos[j], videos[j] = x) ;
                                    return videos;
                                }();
                            }
